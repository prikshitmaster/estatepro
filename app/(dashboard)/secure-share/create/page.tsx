  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const link = await createSecureShareLink({
        ...
        watermark_text:    watermarkText.trim() || "PROTECTED · ESTATEPRO",
      });
      ...
      <div className="p-4 sm:p-6 max-w-lg mx-auto pb-24 sm:pb-6">
        <div className="bg-white rounded-2xl p-8 text-center" style={{ border: '1px solid #EEF1F6', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              onClick={() => {
                const msg = encodeURIComponent(`Hi, here are the property details you requested:\n\n${title}\n${url}`);
                window.open(`https://wa.me/?text=${msg}`, "_blank");
              }}
              className="w-full py-3 text-white text-sm font-semibold rounded-xl flex items-center justify-center gap-2"
      ...
      <form onSubmit={handleCreate} className="space-y-4">
        ...
        <Card>
          ...
          <div className="space-y-4 mt-3">
            ...
            {/* Watermark */}
            <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #EEF1F6' }}>
              <div className="flex items-center justify-between py-3 px-4" style={{ background: '#F8F9FB' }}>
                <div>
                  <p className="text-xs font-semibold" style={{ color: '#374151' }}>Watermark overlay</p>
                  <p className="text-[11px] mt-0.5" style={{ color: '#9CA3AF' }}>
                    {watermark ? 'Tiled text shown on all images & videos' : 'Viewer sees clean images — no watermark'}
                  </p>
                </div>
                <Toggle on={watermark} onToggle={() => setWatermark((v) => !v)} accent={ACCENT} />
              </div>
              {watermark && (
                <div className="px-4 pb-4 pt-3" style={{ background: '#FAFBFF', borderTop: '1px solid #EEF1F6' }}>
                  <Label>Watermark text</Label>
                  <input
                    type="text"
                    value={watermarkText}
                    onChange={(e) => setWatermarkText(e.target.value)}
                    placeholder="e.g. Prikshit Realty · PROTECTED"
                    maxLength={40}
                    className="w-full px-3 py-2 rounded-xl text-sm focus:outline-none"
                    style={{ border: `1px solid ${ACCENT}40`, color: '#1A1D23', background: `${ACCENT}06` }}
                  />
                  <p className="text-[11px] mt-1.5" style={{ color: '#9CA3AF' }}>
                    This text tiles across all images — use your name or agency name
                  </p>
                  {/* Live preview */}
                  <div
                    className="mt-2 rounded-lg px-3 py-2 overflow-hidden text-center"
                    style={{ background: '#1A1D23', fontSize: 11, fontWeight: 800, letterSpacing: '0.12em', color: 'rgba(255,255,255,0.38)', textTransform: 'uppercase', textShadow: '1px 1px 4px rgba(0,0,0,0.9)' }}
                  >
                    {watermarkText || 'PROTECTED · ESTATEPRO'} &nbsp;·&nbsp; {watermarkText || 'PROTECTED · ESTATEPRO'} &nbsp;·&nbsp; {watermarkText || 'PROTECTED · ESTATEPRO'}
                  </div>
                </div>
              )}
            </div>
